...
/// HTTP-based offline queue service
...
class OfflineQueueService extends ChangeNotifier {
  ...
  /// Flush queued items via HTTP API
  Future<int> flush() async {
    ...
    debugPrint('=== OFFLINE QUEUE FLUSH STARTED (HTTP) ===');
    ...
    for (final item in itemsToProcess) {
      try {
        ...
        // Submit via HTTP API
        final response = await _apiClient.post('/feedback', body: {
          ...item.payload,
        ...
        if (response.isSuccess) {
          debugPrint('✅ Successfully submitted! Document ID: ${response.data?['id']}');
          _queue.remove(item);
          success++;
          _successfulSyncs++;
        } else {
        ...
        debugPrint('❌ ERROR submitting item ${item.id}: $e');
...
class OfflineQueue {
  ...
  // flush queued items via API; returns number of successful items
